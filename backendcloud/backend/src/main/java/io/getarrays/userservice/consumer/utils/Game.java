...
public class Game extends Thread {
    ...
    private void updateRating () {
        if ("A".equals(loser)) {
            WebSocketServer.userRepo.updateRatingByUsername(-3, playerA.getId());
            WebSocketServer.userRepo.updateRatingByUsername(5, playerB.getId());
        }
        else if ("B".equals(loser)) {
            WebSocketServer.userRepo.updateRatingByUsername(5, playerA.getId());
            WebSocketServer.userRepo.updateRatingByUsername(-3, playerB.getId());
        }
    }
    ...
    private void sendMove(Long fromId, Long toId, Set<Character> opponentOperate) {
        ...
        if (WebSocketServer.users.get(toId) != null)
    ...
    private final TimerTask ifExit = new TimerTask() {
        @Override
        public void run() {
    ...
    private void judgeGameOver () {
        ...
        timer.schedule(ifExit, new Date(), 1000);
        label:
        while (true) {
            switch (status) {
                case "playing":
