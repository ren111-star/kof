const GIF = function () {
    ...
    let interlaceOffsets = [0, 4, 2, 1]; // used in de-interlacing.
    let interlaceSteps = [8, 8, 4, 2];
    ...
    function parseColourTable(count) { // get a colour table of length count  Each entry is 3 bytes, for RGB.
        let colours = [];
        for (let i = 0; i < count; i++) { colours.push([st.data[st.pos++], st.data[st.pos++], st.data[st.pos++]]) }
        return colours;
    }
    ...
    function loadGif(filename) { // starts the load
        ...
        ajax.send();
        ajax.onerror = function () { error("File error") };
        this.src = filename;
        this.loading = true;
    }
